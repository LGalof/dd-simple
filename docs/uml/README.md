...
| Current Prisma ER Overview | [source](er-current-prisma.puml), [SVG](er-current-prisma.svg) | Grouped Prisma entities for reference data, user/game data, and character state. |
| Room Realtime Sequence | [source](sequence-room-realtime-sync.puml), [SVG](sequence-room-realtime-sync.svg) | Tactical board snapshot synchronization through Socket.IO and persisted `Room.boardState`. |
| Dashboard Room Dice Sequence | [source](sequence-dashboard-room-dice.puml), [SVG](sequence-dashboard-room-dice.svg) | Character Dashboard dice roll persistence followed by Socket.IO room announcement. |
| Room Session Activity | [source](activity-room-session.puml), [SVG](activity-room-session.svg) | User-facing create/join room flow and in-session room activities. |
