...
BEGIN
    IF EXISTS (
        SELECT 1
        FROM pg_class
        WHERE relname = 'character_feature_choice_selections_characterId_sourceType_sour'
    ) AND NOT EXISTS (
        SELECT 1
        FROM pg_class
        WHERE relname = 'character_feature_choice_selections_characterId_sourceType__key'
    ) THEN
        ALTER INDEX "character_feature_choice_selections_characterId_sourceType_sour"
            RENAME TO "character_feature_choice_selections_characterId_sourceType__key";
    END IF;
END $$;
